...
internal func exerciseOne() {
    ...
    let streetAddress = userData["results"][0]["location"]["street"].stringValue
    ...
    print("\(firstName) \(lastName) lives at \(streetAddress) in \(cityName), \(stateName), \(postCode). If you want to contact \(titleName). \(lastName), you can email \(emailAddress) or call at \(cellNum).")
...
internal func exerciseThree() {
    ...
    for movies in allMoviesData {
        let newMovie = Movie(json: movies)
    ...
    for movie in allMovies {
        let rightsOwner = movie.rightsOwner
